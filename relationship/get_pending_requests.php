...
WHERE (F.Users_sent_ID = ? AND F.Status_of_Request = 'pending')
UNION 
SELECT U.ID, U.first_name, U.family_name FROM users U 
LEFT JOIN friends F ON U.ID = F.Users_sent_ID 
WHERE (F.Users_recieved_ID = ? AND F.Status_of_Request = 'pending')
...
if($num_rows==0){
    $array_response["status"]= "Pending requests not found !";
} else{
