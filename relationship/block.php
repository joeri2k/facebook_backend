...
if($num_rows1 !=0){
    ...
    SET  Status_of_Request = 'blocked',
        Users_sent_ID = ?,
        Users_recieved_ID = ?
    WHERE (Users_sent_ID = ? AND Users_recieved_ID = ?)");
    $query ->bind_param("ssss", $my_id,$user_id, $user_id, $my_id);
    $query -> execute();
    $query -> store_result();
} else{

$query = $mySqli ->prepare("INSERT INTO friends(where Users_sent_ID, Users_recieved_ID, Status_of_Request) 
VALUES(?,?,'blocked')");
